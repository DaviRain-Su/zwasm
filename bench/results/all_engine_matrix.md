...
5. **Two real codegen anomalies (→ debt, not design trade):**
   - **`memmove`**: zwasm-jit (254 ms) is **slower than zwasm-interp** (138 ms)
     and **~15× wasmtime** (17 ms). A JIT slower than its own interpreter is a
     codegen defect, not the §1.3 trade — strong signature of a
     `memory.copy`/byte-loop fast-path the interpreter has and the JIT lacks.
   - **`base64`**: zwasm-jit **~13×** wasmtime (770 vs 58 ms), far past the
     1.5–3.9× seen elsewhere — same byte-manipulation family.
   Filed **D-284** (nbody harness gap) and **D-285** (memmove/base64 jit
   byte-loop codegen). These are measured deficiencies; per ADR-0153 they are
   investigation candidates, not silent accepts.

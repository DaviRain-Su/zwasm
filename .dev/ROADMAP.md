...
| 9.5  | ARM64 emit (NEON): SIMD load/store + lane access + integer arithmetic. Per LOOP.md chunk granularity (≤ 400 LOC src + ≤ 250 LOC tests per chunk), splits into 9.5-a (NEON encoder foundation `inst_neon.zig`: LDR Q / STR Q / ORR.16B / DUP.4S / ADD.4S [x] this commit), 9.5-b (per-op handlers: v128.load/store + i32x4.splat + i32x4.add + ZirOp dispatch wiring + `Allocation.shape_tags` population from ZirOp metadata), 9.5-c (extract/replace_lane + remaining int arith op shapes). | [ ] (9.5-a `[x]`) |

...
| 9.5  | ARM64 emit (NEON): SIMD load/store + lane access + integer arithmetic. Splits into 9.5-a (encoder foundation `[x]`), 9.5-b-i (shape-tag predicate + populator `[x]`), 9.5-b-ii (compute() integration `[x]`), 9.5-b-iii (per-op handlers + dispatch `[x]`), 9.5-c-i (Q-form v128 spill helpers `[x]`), 9.5-c-ii (op_simd handlers refactored to use q* helpers — v128 spill-aware via stage_idx 0/1; lhs binop pattern reuses stage_idx 0 for result since lhs is consumed; SPILL-EXEMPT markers retained only for i32-addr GPR sites pending separate GPR-spill-aware refactor `[x]`), 9.5-c-iii (extract/replace_lane handlers + remaining int-arith op shapes). | [ ] (9.5-a/b/c-i/c-ii `[x]`) |

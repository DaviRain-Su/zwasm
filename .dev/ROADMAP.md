...
| 14.3 | `.github/workflows/nightly.yml` — fuzz + spec-bump + proposal-watch. **PARTIAL (D-256)**: `nightly.yml` BUILT (`17e3b6f1`) with 2/3 legs — fuzz campaign (`gen_fuzz_corpus.sh campaign` → `zig build fuzz-campaign`, ~2000 smith modules; harness `6c80c229`+`16584c1c` = parse/validate/instantiate crash-fuzz) + proposal-watch freshness (`check_proposal_watch.sh`). REMAINING: **spec-bump** — needs a Wasm-3.0 pin-target decision first (the recorded pin is Phase-9 `wg-2.0`, no enforced SHA); overlaps the proposal-watch review. | [ ]  |

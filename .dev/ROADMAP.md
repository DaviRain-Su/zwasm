...
| 11.1 | WASI 0.1 (preview1) **core subset (21/46)** — stdio + basic file ops + preopen discovery, sufficient for the realworld corpus (50 fixtures Mac+Linux; Windows subset 25). **The "full" label was an OVERCLAIM — corrected 2026-06-05 (ADR-0161; audit `src/api/wasi.zig:285`).** 25 syscalls remain unimplemented (all 9 sockets, `fd_readdir`, 7 `path_*`, `fd_pread/pwrite/sync/datasync/advise/allocate`, file-meta setters, `clock_res_get`, `proc_raise`). The **21→46 completion** is scheduled under the **ADR-0161 WASI program** (Phase 16 完成形).              | [x] subset            |

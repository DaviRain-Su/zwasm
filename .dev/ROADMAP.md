...
| 9.5  | ARM64 emit (NEON): SIMD load/store + lane access + integer arithmetic. Sub-rows: 9.5-a (encoder foundation `[x]`), 9.5-b-i/ii/iii (shape-tag pipeline + per-op handlers + dispatch `[x]`), 9.5-c-i/ii (Q-form v128 spill helpers + op_simd refactor `[x]`), 9.5-c-iii (i32x4 extract/replace_lane via UMOV/INS `[x]`), 9.5-c-iv (i8x16/i16x8/i64x2 add+sub via shared `emitV128Binop` helper + ADD/SUB encoder family with size-field discriminator `[x]`), 9.5-c-v (mul shapes — i8x16/i16x8/i32x4 via NEON MUL Vd.<T> + i64x2.mul via synthesis since no NEON 2D-MUL instruction exists; +remaining lane-access shapes for i8x16/i16x8/i64x2/f32x4/f64x2). | [ ] (9.5-a/b/c-i/c-ii/c-iii/c-iv `[x]`) |

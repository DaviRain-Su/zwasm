...
| 12.4    | Cold-start bench-delta: AOT load + first-call vs JIT compile + first-call **≥30%** on ≥3 **compute (zero-import) fixtures** (re-scoped from "v1-class" per ADR-0140 — WASI-importing fixtures run on neither path until JIT-WASI/D-244; the SIMD corpus + zero-import compute kernels run AOT today). The ADR-0040-deferred §9.8b/8b.3 bench obligation; threshold from `private/notes/p8-8b3-aot-survey.md`. **Unblocked** (§12.3b compute subset DONE). | [ ]  |

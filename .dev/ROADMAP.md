...
| 9.6  | ARM64 emit (NEON): SIMD comparison + shuffle + float arithmetic + conversion. Sub-rows: 9.6-a (f32x4 / f64x2 binary FP arith — FADD/FSUB/FMUL/FDIV `[x]`), 9.6-b (FP unary — sqrt/abs/neg/ceil/floor/trunc/nearest via FABS/FNEG/FSQRT/FRINT* + shared `emitV128Unop` helper `[x]`), 9.6-c-i (FP min/max via FMIN/FMAX, NaN-propagating `[x]`), 9.6-c-ii (FP pmin/pmax synthesis via FCMGT + BSL — pseudo-min/max with zero-on-equal-magnitude semantics; A64 has no direct instruction), 9.6-d (int compare — i*x*.eq/ne/lt/gt/le/ge signed+unsigned via CMEQ/CMGT/CMHI/CMGE/CMHS), 9.6-e (FP compare — f*x*.eq/ne/lt/gt/le/ge ordered via FCMEQ/FCMGT/FCMGE), 9.6-f (shuffle — i8x16.shuffle via TBL, swizzle via TBL-direct), 9.6-g (conversion — trunc_sat / convert / narrow / extend). | [ ] (9.6-a/b/c-i `[x]`) |

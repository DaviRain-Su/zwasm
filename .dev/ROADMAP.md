...
| 9.5  | ARM64 emit (NEON): SIMD load/store + lane access + integer arithmetic. Splits into 9.5-a (NEON encoder foundation `inst_neon.zig`: LDR Q / STR Q / ORR.16B / DUP.4S / ADD.4S `[x]`), 9.5-b-i (`zir.isSimdZirOp` predicate + `regalloc.populateShapeTags` helper walking `func.instrs` to build per-vreg shape map; `regalloc.deinit` extended to free `shape_tags` `[x]`), 9.5-b-ii (per-op emit handlers: v128.load/store + i32x4.splat + i32x4.add + ZirOp dispatch wiring in arm64/emit.zig + `compute()` integration calling populateShapeTags), 9.5-c (extract/replace_lane + remaining int arith op shapes). | [ ] (9.5-a + 9.5-b-i `[x]`) |

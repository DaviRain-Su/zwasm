...
| 9.6  | ARM64 emit (NEON): SIMD comparison + shuffle + float arithmetic + conversion. Sub-rows: 9.6-a (f32x4 / f64x2 binary FP arith — FADD/FSUB/FMUL/FDIV `[x]`), 9.6-b (FP unary — sqrt/abs/neg/ceil/floor/trunc/nearest), 9.6-c (FP min/max/pmin/pmax — Wasm-spec quirky NaN handling), 9.6-d (int compare — i*x*.eq/ne/lt/gt/le/ge signed+unsigned via CMEQ/CMGT/CMHI/CMGE/CMHS), 9.6-e (FP compare — f*x*.eq/ne/lt/gt/le/ge ordered via FCMEQ/FCMGT/FCMGE), 9.6-f (shuffle — i8x16.shuffle via TBL, swizzle via TBL-direct), 9.6-g (conversion — trunc_sat / convert / narrow / extend). | [ ] (9.6-a `[x]`) |

...
| 15.3 | **Class-aware allocator** (dual-pool GPR/FP, liveness type-tagging, tighter `spillBytes()`; ADR-0038/0040). Exit: ≥3% FP-heavy; combined class-aware + §15.4 SIMD aggregate ≥10% on 3 v1-class fixtures, validated at §15.P vs v1 main (§15.2 mov-reduction folded out per ADR-0149 — assess this ≥3% bar against §15.3's OWN measurement; v2's spill traffic is low, so confirm headroom before committing). | [ ]  |
...
| 15.P | Phase 15 close — bench parity validation (no unexplained regression vs v1 main; class-aware (§15.3) + SIMD (§15.4) deltas met; §15.2 mov-reduction was empirically unreachable per ADR-0149, its residual store-then-reload peephole landed here opportunistically if a net win) + 3-host reconcile (D-245-dependent) + widget 15 → DONE + Phase 16 inline expand. | [ ]  |

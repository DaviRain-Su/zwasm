...
| 10.J    | **Native Zig API (ADR-0109 Accepted 2026-05-25)** — `src/zwasm.zig` rewrite per `docs/zig_api_design.md`: Engine + Linker + TypedFunc + Memory slice view + Caller ctx + full Trap error set + allocator strict-pass, dropping the ADR-0025 c_api veneer. Internal rename `runtime.Runtime` → `runtime.JitRuntime` lands first (mechanical; ABI-preserving) so subsequent Phase 10 chunks (10.M/R/TC/E/G) don't compound the rename cost. **Gating sub-chunk**: pre-impl codebase-investigation (subagent-driven; enumerates every site needing change) + execution plan + integrated test strategy (covers regression detection / happy path / edge cases so "other tests pass while Zig API is broken" cannot happen) — user-reviewed plan doc gates the first J.* impl chunk. Closes D-075 on impl complete (= ADR-0109 Status → `Closed (implemented)` per its Removal condition).                                       | [ ]    |

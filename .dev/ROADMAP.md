...
| 15.2 | **Redundant spill-reload elimination** (re-targeted from slot-alias coalescing per **ADR-0149**: structural read of `arm64/{emit,gpr}.zig` proved the gpr helpers already elide all reg-resident movs + v2 emits NO vreg-to-vreg movs, so slot-alias coalescing has ~0 headroom). New mechanism: a spilled vreg used N× re-emits `LDR` each use → cache "vreg→staging-reg" during emit, skip the reload (invalidate at every call + branch target — W54). Coalescer scaffolding (ADR-0035/0036) left dormant. **First step empirical**: measure reload headroom on loop-heavy fixtures. Exit: ≥5% bench-delta on loop-heavy fixtures (or revisit ADR-0149 if headroom < target). | [ ]  |
| 15.3 | **Class-aware allocator** (dual-pool GPR/FP, liveness type-tagging, tighter `spillBytes()`; ADR-0038/0040). Exit: ≥3% FP-heavy; combined §15.2-reload-elim + class-aware (+ §15.4 SIMD) aggregate ≥10% on 3 v1-class fixtures, validated at §15.P vs v1 main (the ex-"coalescer" component is now §15.2 reload-elim per ADR-0149). | [ ]  |

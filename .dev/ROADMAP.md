...
| 7.5  | spec test pass=fail=skip=0 via ARM64 JIT on Mac aarch64 host (drives every Wasm 1.0 + 2.0 op the interp covers). `skip=0` semantics per ADR-0029: `skip-impl` (implementation-gap + test-shape) のみカウント、`skip-adr-<id>` (proposal-skip ADR 経由の deliberate 除外) は別 tally。 | [ ]            |
...
| 7.8  | spec test pass=fail=skip=0 via x86_64 JIT on Linux x86_64 AND Windows x86_64 hosts. `skip=0` semantics per ADR-0029 (= 7.5 と同じ; `skip-impl` のみカウント、`skip-adr-<id>` は別 tally)。 | [ ]            |

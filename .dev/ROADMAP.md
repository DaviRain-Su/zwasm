...
| 12.5    | `.cwasm` stack-map section: per-callsite entries serialised in the JIT-mode shape, gated `Module.needs_gc_heap` (empty/zero-overhead for Wasm 1.0/2.0) per ADR-0117 I4. **Walker-side GC rooting is Phase 15 (ADR-0135)** — this row is format/emission only. (Independent of §12.3b; **next unblocked row** per ADR-0139.)   | [ ]  |

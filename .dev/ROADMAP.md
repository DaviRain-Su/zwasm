...
| 14.3 | `.github/workflows/nightly.yml` — fuzz + spec-bump + proposal-watch (`.dev/proposal_watch.md` quarterly cadence). **PARTIAL (D-256)**: fuzz crash-harness BUILT (`6c80c229` — `test/fuzz/fuzz_loader.zig` + `gen_fuzz_corpus.sh` + `test-fuzz` in test-all). REMAINING: spec-bump checker (still absent) + `nightly.yml` wiring the campaign + (extension) differential oracle. | [ ]  |

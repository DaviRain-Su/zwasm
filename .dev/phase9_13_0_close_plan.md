...
| 7' | **W3.b-2** arm/disarm callsite integration | `emit` | NO | 5 `sigsetjmp` callsites on Windows arm replaced with `arm(RecoveryInfo{...})` + `disarm()`; recovery PC/SP capture via MSVCRT `_setjmp` analogue (Step 0 verification needed) |

...
| B143 | §9.12-E SKIP-CROSS-MODULE-IMPORTS scope survey. ~100 sites across 8 manifests: imports (40-50), linking (30-40), table_* / elem (35-40), memory_grow / data (10-15), globals (10-15), ref_func (3-5). ADR-0066 bridge thunks already implemented (cross-module func calls work in JIT). Plan: 4 chunks — A (func dispatch + link-typecheck, ~25), B (global R/W, ~15), C (table get/set/init/copy, ~35-40), D (memory load/store/grow, ~15-20). Survey at private/notes/p9_12-E-B143-cross-module-imports-survey.md. | (survey-only) |
| **B144** | §9.12-E Chunk A — cross-module func import dispatch. The hasUnbindableImports + hasIncompatibleImportType paths already gate the link-time type-check for func imports against registered modules; what's missing is wiring the IMPORT-resolution to drive actual cross-module invokes through the ADR-0066 bridge thunks during assert_return execution. Investigate: does the existing cross-module resolver (per ADR-0065 / chunk (c)-2.3) already pass these spec tests when the import binding succeeds? If yes, the manifest regen skipping is overly defensive — re-run regen + measure. If no, identify the specific assert_return cases that fail and wire dispatch. | **NEXT** |

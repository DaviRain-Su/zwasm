...
| B153 | §9.12-E Chunk B Part 8 — `computeGlobalsLayout` walks imports prefix; result indexed by FULL wasm global index space. Empty-fn path now returns `(num_imports + defined)`-sized arrays. Non-empty-fn path stays defined-only (inconsistency invisible today since hasUnbindableImports gates global imports). Behaviour-preserving on current corpus. | `<this commit>` |
| **B154** | §9.12-E Chunk B Part 9 — non-empty-fn path symmetry. Update `runner.zig::compileWasm`'s non-empty-fn globals_offsets allocation (lines ~747-768) to mirror computeGlobalsLayout's shape: alloc `(num_global_imports + defined_globals_count)`, pre-populate imports prefix from imports_buf's global entries with their valtypes + sequential 8-byte offsets, then defined entries with `off` starting at `num_global_imports * 8`. Behaviour-preserving on current corpus (num_global_imports = 0). | **NEXT** |

...
| B146 | §9.12-E Chunk B Part 1 — new `test/spec/spectest_catalog.zig` (D-153 Part 1). Static `non_func_exports` array enumerating global_i32/i64/f32/f64 + table(10,20) + memory(1,2) per the reference interpreter's spectest.ml. `findNonFuncExport(name)` lookup helper + 3 unit tests. No behaviour change; subsequent chunks consume the catalog. | `<this commit>` |
| **B147** | §9.12-E Chunk B Part 2 — wire the spectest catalog into `hasUnbindableImports`. For non-func imports from spectest, check `spectest_catalog.findNonFuncExport(imp.name)` — if present AND kind matches AND (for global) valtype matches → mark bindable. The full instantiation-time storage wiring is Part 3 (B148); this chunk's goal is just to stop emitting SKIP for the recognized shapes so subsequent runner steps surface what else fails. Expect FAIL (not SKIP) for some sites after this lands — they'll show the next architectural gap to plug. | **NEXT** |

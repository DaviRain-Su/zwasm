...
| B147 | §9.12-E Chunk B Part 2 — `isSpectestNonFuncBindable(imp) bool` predicate in `spec_assert_runner_base.zig`. Consults catalog; returns true iff module=spectest + name in catalog + kind matches + (for globals) valtype matches. `hasUnbindableImports` NOT yet changed; predicate sits ready for B148 wire-up. | `<this commit>` |
| **B148** | §9.12-E Chunk B Part 3 — wire isSpectestNonFuncBindable into `hasUnbindableImports`'s Path #2: allow recognized spectest non-func imports through (don't return true for them). Behaviour-change cycle: some sites will FAIL instead of SKIP since storage isn't populated yet. Measure the FAIL count vs prior SKIP count to size B149+ (storage population: scratch_globals slot init from catalog.init_bits, growable_memory pool, scratch_funcptrs region for table). Architectural per ADR-0071 §Q5; if FAILs cascade or surface a structural blocker, this becomes a multi-cycle piece. | **NEXT** |

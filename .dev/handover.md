...
| B142 | §9.12-E B139 Part (c) — spectest kind-mismatch arm. Func imports targeting `spectest.{global_i32,global_i64,global_f32,global_f64,table,memory}` or literal `spectest.unknown` → PASS. Closes the residual 4 SKIP-NO-LINK-TYPECHECK sites (imports.13/33/34/35). Net: 25348 → 25352 passed (+4), skip-adr 473 → 469. SKIP-NO-LINK-TYPECHECK fully discharged on wasm-2.0 corpus. | `<this commit>` |
| **B143** | §9.12-E continuation — measure SKIP-CROSS-MODULE-IMPORTS state (100 + ~66 cascade sites in skip-impl breakdown) and pick first sub-chunk. Per the substrate-audit context this is the largest remaining workstream; survey first (Step 0) before implementation. Also: update skip_impl_history.yaml ratchet entry to reflect 192 stable + the +26 PASS delta. | **NEXT** |

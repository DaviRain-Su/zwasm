...
| B145 | §9.12-E B143/144 distribution measured: imports=38, elem=19, data=18, linking=16, table_grow=2, global=2, table=1, memory_grow=1. D-153 filed with architectural discharge plan (~400-600 LOC across spec_assert_runner_base.zig + new spectest_catalog module; ADR-grade per ADR-0065 §A4 / ADR-0066 / ADR-0071 §Q5). | `<this commit>` |
| **B146** | §9.12-E Chunk B Part 1 — start the D-153 discharge with the smallest piece: write the spectest catalog as a static const in spec_assert_runner_base.zig (or new test/spec/spectest_catalog.zig), enumerating the 6 non-func exports with their types + values. No behaviour change yet; subsequent chunks (B147+) extend hasUnbindableImports + wire instantiation storage. | **NEXT** |

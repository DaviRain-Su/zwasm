...
| B151 | §9.12-E Chunk B Part 6 — cascade discovery cycle. Attempted to extend computeGlobalsLayout to include imports prefix; reverted on discovery that `applyDefinedGlobalsInit` (runner.zig:1015, :1054) asserts `globals_decoded.len == globals_offsets.len`. The cascade requires: (1) extend applyDefinedGlobalsInit signature with num_global_imports, (2) shift its indexing by + num_global_imports, (3) update 2 spec-runner callers, (4) then extend globals_offsets in compileWasm. Multi-cycle ordering needed. | `<this commit>` |
| **B152** | §9.12-E Chunk B Part 7 — extend `applyDefinedGlobalsInit(..., num_global_imports: u32)` first, update its indexing to `globals_offsets[num_global_imports + gi]`, fix the length assertion to `globals_offsets.len - num_global_imports == defined_count`. Update both call sites (spec_assert_runner_non_simd + simd_assert_runner) to pass 0 (existing modules have no global imports). Behaviour-preserving prep step before compileWasm's globals_offsets resize lands. | **NEXT** |

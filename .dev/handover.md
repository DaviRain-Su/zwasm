...
| B140 | §9.12-E B139 Part (a) — `getExportFuncType` helper in `engine/export_lookup.zig`. Looks up an exported func by name, resolves its `FuncType` via the type section. Handles both defined and imported funcs in the func index space. Caller-owned param/result slices. Unit test covers happy path. | `<this commit>` |
| **B141** | §9.12-E B139 Part (b) — import-vs-export comparison loop. For each `assert_unlinkable` fixture, walk the importer's import section; for each func import, look up the registered exporter (via the importer's `(import "module" "field")` module name + the runCorpus `registered` map), call `getExportFuncType` on the exporter's wasm_bytes, compare param/result vectors vs the importer's `it.payload.func_typeidx`-resolved type. On mismatch → return true (= PASS the assert_unlinkable). Wire as `applyAssertUnlinkable` callback. | **NEXT** |

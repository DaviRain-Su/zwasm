...
| B150 | §9.12-E Chunk B Part 5 — `CompiledWasm.num_global_imports: u32` field added + populated by both empty-fn and non-empty-fn paths. Pure data exposure step; `globals_offsets` still keys by defined idx. No behaviour change for existing modules (`num_global_imports` is 0 for everything reaching compileWasm today). runner.zig stays at 1999 LOC (1 line under D-141's 2000 hard cap). | `<this commit>` |
| **B151** | §9.12-E Chunk B Part 6 — extend `globals_offsets` to size `(num_global_imports + defined)`. Pre-populate import slots at `[0..num_global_imports]` with sequential 8-byte offsets. Shift defined offsets to start at `num_global_imports * 8`. Verify test-all stays green (no module reaching compileWasm has global imports today, so this should be a structural-only change). Subsequent (B152) wires hasUnbindableImports to accept spectest non-func + populates `scratch_globals[]` from catalog at instantiation. | **NEXT** |

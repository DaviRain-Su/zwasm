...
| D-035 | code   | blocked-by: parser/validator gap for block-with-results / multi-value blocks | `block.wast` の `block.0.wasm` を spec_assert に取り込むと `compileWasm: ... → BadBlockType`。`src/validate/validator.zig:readBlockType` は Wasm 1.0 の単純 blocktype (-64..-1〜-4 or typeidx) しかデコードしない。`block.wast` は Wasm 2.0 multi-value blocks (`(block (param i32) (result i32))`) を多用するため、parser/validator 側に typeidx-based blocktype path の拡張が必要。**Discharge** = (a) parse 側で block-typeidx を Wasm 2.0 multi-value 仕様で正しく decode、(b) validator 側で param/result count を type-section 経由で resolve、(c) emit 側 (op_control.emitBlock) は merge_top_vreg 機構を multi-value に拡張。範囲は中規模 — 1 chunk で抽出可能だが ROADMAP §A の Wasm 2.0 multi-value 取り込みが正式 row として open している前提。Spec-assert corpus を `block`/`switch`/`select` 等に広げるためのキーピース。 | 2026-05-06 | 2026-05-06 | `src/validate/validator.zig:readBlockType`, `src/parse/sections/code.zig`, `src/engine/codegen/arm64/op_control.zig:emitBlock` |

...
- **Status**: Superseded by ADR-0109 (Accepted 2026-05-25). The original "minimum subset" target shape (Runtime / Module / Instance / Value as thin c_api veneer) is fully replaced by ADR-0109's native Zig API (Engine + Linker + TypedFunc + Memory slice view, first-principles independent of the c_api binding). This ADR remains in-tree as **design lineage** — its Context / Alternatives / Implementation-phase analysis seeded the eventual direction. The "Zig consumers deserve a curated facade" layering goal persists; the concrete shape pivots per ADR-0109. D-075 re-scoped from "blocked-by ADR-0109 Accept" to impl tracker (ROADMAP §10 / 10.J).
- **Date**: 2026-05-05
...
| 2026-05-25 | (this commit) | gap | **Status: Superseded by ADR-0109 (Accepted 2026-05-25)**. The "minimum subset" target shape (Runtime / Module / Instance / Value as thin c_api veneer) is fully replaced by ADR-0109's first-principles native Zig API (Engine + Linker + TypedFunc + Memory). cw v1 dogfooding (2026-05-24) surfaced that the c_api-veneer minimum subset is unusable for Zig consumers (allocator strict-pass, host imports, NaN-boxing-friendly Value all blocked); user direction at Phase 10 open authorized the ADR-0109 inversion + impl scheduling. The Phase 16 deferral that the 2026-05-11 row above carried is **OBSOLETE** — ROADMAP §10 / 10.J is the new impl carrier. This ADR stays in-tree as design lineage (Context / Alternatives analysis seeded the direction); body text is NOT amended further (per ARCHIVED-IN-PLACE discipline). For consumer-facing API spec see `docs/zig_api_design.md`; for impl tracking see D-075 (re-scoped) + ROADMAP §10 / 10.J. |

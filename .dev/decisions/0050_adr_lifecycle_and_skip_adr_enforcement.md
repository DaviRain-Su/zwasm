...
| 2026-05-19 | `<backfill>` | **Amend (add D-3 + D-4 — Phase 9 completion substrate audit per ADR-0071)**. Integrate skip-impl one-way ratchet into the ADR lifecycle: (D-3) treat `bench/results/skip_impl_history.yaml` as the ratchet substrate; any PR that changes skip-impl counts in the increasing direction must be justified by an ADR and registered with `exempt: <ADR-NNNN>`. (D-4) add `scripts/check_skip_impl_ratchet.sh` to the pre-push gate (lands in §9.12-A). Skeleton; full body to be expanded in §9.12-pre. |

...
| 2026-05-25 | (this commit) | **Chunk α scaffold (`src/engine/codegen/shared/table_storage.zig`) deleted as orphan**. Investigation triggered by porting cw v1's Zig 0.16 lazy-decl-analysis test-discovery lesson: the file had 1 test block (`"mirrorWrite stubs are callable"`) but **zero importers** in `src/` or `test/`, so the test never ran — exactly the failure mode that hid `nanoTimestamp` removal on cw v1's `main` for months. Inspection of actual `src/engine/codegen/{arm64,x86_64}/op_table.zig` writing handlers (Set/Copy/Init/Grow/Fill) confirms the dual-view sync is **already implemented inline per arch** via direct STR triplet (refs slot + funcptr_base + typeidx_base) — chunks β/γ never wired the central helper because inline per-arch shipped first and the §9.12-F cohort close didn't surface a retrofit need. Per the companion `.claude/rules/dual_view_table_sync.md` §"When the rule dissolves" (Option 1: unified storage), the helper was supposed to be rewritten as a no-op wrapper or deleted after the §9.12 substrate audit (`43d82eb5`); neither happened, leaving an orphan scaffold + a rule whose `audit_scaffolding §F grep` was never implemented. **Regression detection holds** via (1) wasm-2.0 spec corpus `table_*` fixtures on 3 hosts (the original D-126 catcher), and (2) `test/edge_cases/p9/table_storage_sync/` 5 fixtures wired into `test-edge-cases` step (`build.zig:258 run_edge_p9.addArg("test/edge_cases/p9")`). **Replacement gate**: `scripts/audit_table_sync.sh` (this commit) lifts the aspirational rule into an actual grep — for each writing handler in arm64 / x86_64 op_table.zig, requires references to both `tables_ptr_off` (refs base) AND `tables_jit_ci_ptr_off` (funcptr/typeidx base); FAILs if any writer is unpaired. Rule file `.claude/rules/dual_view_table_sync.md` deleted same commit (discipline now enforced by script, not aspirational prose). ADR-0068 `Status: Accepted` stays — the *design* (dual-view storage; mirror writes; ABI-stable) is correct; only the central-helper enforcement mechanism was abandoned in favor of inline per-arch + the new grep gate. |

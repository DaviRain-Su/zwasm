...
// VerifyError + verify + verifyWith extracted to
// `regalloc_verify.zig` per ADR-0097; re-exported below after the
// compute family so callers continue to reach `regalloc.VerifyError`
// / `regalloc.verify` identically.
...
// verify + verifyWith moved to `regalloc_verify.zig` per ADR-0097.
const verify_mod = @import("regalloc_verify.zig");
pub const VerifyError = verify_mod.VerifyError;
pub const verify = verify_mod.verify;
pub const verifyWith = verify_mod.verifyWith;
...
// verify tests moved to `regalloc_verify.zig` per ADR-0097.
...
// verifyWith tests moved to `regalloc_verify.zig` per ADR-0097.

...
pub const op_tag: ZirOp = .@"local.tee";
pub const wasm_level: ?WasmLevel = .v1_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_local_tee,
    .lower = lower_local_tee,
    .interp = interp_local_tee,
};

fn validate_local_tee() collector.DispatchError!void {
    return error.NotMigrated;
}
fn lower_local_tee() collector.DispatchError!void {
    return error.NotMigrated;
}
fn interp_local_tee() collector.DispatchError!void {
    return error.NotMigrated;
}

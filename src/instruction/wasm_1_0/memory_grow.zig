...
pub const op_tag: ZirOp = .@"memory.grow";
pub const wasm_level: ?WasmLevel = .v1_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_memory_grow,
    .lower = lower_memory_grow,
    .interp = interp_memory_grow,
};

fn validate_memory_grow() collector.DispatchError!void {
    return error.NotMigrated;
}
fn lower_memory_grow() collector.DispatchError!void {
    return error.NotMigrated;
}
fn interp_memory_grow() collector.DispatchError!void {
    return error.NotMigrated;
}
